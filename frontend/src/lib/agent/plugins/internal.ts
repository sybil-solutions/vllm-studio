// Shared, cycle-free plugin discovery helpers. These are the pure dir-walk +
// manifest-parse + dedupe/sort primitives that every asset source and the
// registry reuse. They were factored out of plugin-discovery.ts so the sources
// (and registry, which plugin-discovery.ts in turn delegates to) can depend on
// them WITHOUT importing plugin-discovery.ts — that would create an import
// cycle. Keep this file free of any import from ./registry or ../plugin-discovery.
//
// types.ts stays pure-types-only; this file holds the runtime helpers and is the
// single home for the manifest/dir-walk logic (no copy-paste drift). plugin-
// discovery.ts re-exports the public ones (defaultPluginRoots) for back-compat.
...
export function hasPluginMarker(dir: string): boolean {
  return (
...
export function pluginManifest(dir: string): PluginManifest {
  try {
...
export function marketplaceFromPath(dir: string): string | undefined {
...
export function knownLocalPluginRows(): PluginRow[] {
...
export type PluginDiscoveryState = {
...
export function collectPluginRows(dir: string, depth: number, state: PluginDiscoveryState): void {
...
export function pluginRowFromDirectory(dir: string, codexConfig: CodexConfig): PluginRow {
  const manifest = pluginManifest(dir);
...
export function includesDefaultPluginRoot(roots: string[]): boolean {
  const defaultRoot = path.resolve(path.join(homedir(), ".codex", "plugins"));
  return roots.some((root) => path.resolve(root) === defaultRoot);
}

export function dedupePluginRows(rows: PluginRow[]): PluginRow[] {
...
export function sortPluginRows(rows: PluginRow[]): PluginRow[] {
  return [...rows]
...
function preferredPluginRow(current: PluginRow, candidate: PluginRow): PluginRow {
  ...
  // Prefer the functional variant: a row carrying a usable .mcp.json (so its
  // tools can actually launch) beats an asset-less duplicate. This makes the
  // bundled computer-use (real .mcp.json + skills) win over a bare local helper
  // dir that only ships the .app shell — while still preferring a local
  // clean-room helper when it is the one carrying the MCP (both-present falls
  // through to the local-helper rule below).
