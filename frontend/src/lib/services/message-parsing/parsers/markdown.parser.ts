...
markdownIt.renderer.rules.link_open = (
  tokens: Array<{ attrIndex: (name: string) => number; attrPush: (attr: [string, string]) => void; attrs?: Array<[string, string]> }>,
  idx: number,
  options: Record<string, unknown>,
  _env: unknown,
  self: { renderToken: (tokens: unknown[], idx: number, options: Record<string, unknown>) => string }
) => {
...
export class MarkdownParser implements IMarkdownParser {
  ...
  normalizeForRender(content: string): string {
    ...
    const skipWords = /(?:JavaScript|TypeScript|GitHub|LinkedIn|YouTube|WordPress|iPhone|iPad|MacBook|PowerPoint|PlayStation|OneDrive|OneNote|OutLook)/;
    text = text.replace(
      /([a-z])([A-Z][a-z])/g,
      (match, lower, upper) => {
    ...
    text = text.replace(
      /```mermaid\s*(?:graph\s+)?(?=[A-Z]{1,3}\b)/gi,
      '```mermaid\ngraph '
    );
