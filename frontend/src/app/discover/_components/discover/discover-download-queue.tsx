  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="h-1.5 w-full rounded-full bg-(--card)">
        <div className="h-1.5 rounded-full bg-(--accent-purple) transition-all" style={{ width: `${progress}%` }} />
      </div>
      {download.error && download.status === "failed" && <div className="text-xs text-(--error)">{download.error}</div>}
    </div>
  );
};
