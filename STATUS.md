...
- [x] Inspect stale generated comments in controller process manager.
- [x] Remove boilerplate JSDoc from `controller/src/modules/engines/process/process-manager.ts` without behavior changes.
...
- [ ] Audit comments across the repo and delete stale or irrelevant comments. Current slices remove empty/generated JSDoc blocks from controller source/tooling, exact non-informative `// CRITICAL` headers, stale annotated `CRITICAL` migration labels, generated lifecycle JSDoc noise from the engine coordinator, non-informative `CRITICAL` banners from deploy/global-style files, generated download-manager JSDoc noise, parameter-only download helper JSDoc, boilerplate process utility JSDoc, backend-builder command JSDoc noise, runtime parser default JSDoc noise, and process-manager wrapper JSDoc noise; broader file-by-file audit remains open.
