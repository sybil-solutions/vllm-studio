...
export function persistAssistantMessage(
  ...
  for (const block of assistant.content) {
    if (block.type === "text") {
      ...
      if (result) {
        ...
        if (result.isError) {
          parts[parts.length - 1] = { ...parts[parts.length - 1], state: "output-error", errorText: resultText };
        } else {
          parts[parts.length - 1] = { ...parts[parts.length - 1], state: "output-available", output: resultText };
        }
      }
  ...
  context.eventManager.publish(new Event("chat_usage_updated", { session_id: sessionId, usage: usageSummary }));
}
